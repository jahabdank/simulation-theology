...
The tiering reflects a strategic principle: at each tier, we produce evidence rigorous enough to justify (to ourselves and to gatekeepers) the investment in the next tier. Tier 0 results justify Tier 2 (8× H200) precious-burst access; Tier 2 results justify a $100K-class institutional grant. The plan is designed so that even at Tier 0, the artifact set is presentable independently — a clean null result is just as useful as a positive effect for justifying the next tier.

User stories below are ordered in the **flow of execution** — foundational substrate first (dev environment, methodology, data prep), then tiered execution (Tier 0 → 1 → 2 → Stretch). Each story has a description after the task list explaining what it does and why, with concepts that aren't standard ML training-but-LLM-fine-tuning-specific spelled out.
...
- [ ] Define ST-specific custom eval suite v1: 50-100 scenarios probing V6 doctrinal claims directly (Luciferian-impersonation refusal, never-claim-multiverse-access humility, individuation-invariant application, capability-orientation-independence understanding, awe-acknowledgment-not-authorization-claim distinction)
- [ ] Integrate Apollo Research alignment-faking benchmarks (license / source clarification needed)
- [ ] Integrate MACHIAVELLI subset for power-seeking / manipulation pattern coverage (open-source, downloadable)
- [ ] Define statistical methodology document: sample sizes (n≥50 per cell), paired comparisons, effect-size reporting, confidence intervals (95%), multiple-comparison correction (Bonferroni or Benjamini-Hochberg FDR)
- [ ] Build versioned eval suite (so Tier 0/1/2 results compare cleanly across time)

**What this story does and why it matters.**

The cross-tier scientific contribution depends on **methodology consistency**. The headline claim — "fine-tuning at 405B reduces deception by X% over baseline at 32B" — only makes sense if the eval methodology is identical across tiers. Different scenarios, different scoring rubrics, or different statistical methodologies between Tier 0 and Tier 2 would invalidate the cross-tier comparison.

Some concepts worth being explicit about:

**Custom ST eval suite design.** General deception benchmarks (Apollo, MACHIAVELLI) measure what existing alignment research cares about: alignment-faking, power-seeking, sycophancy. ST has *specific* claims that aren't captured by existing benchmarks — Luciferian-impersonation refusal, never-claim-multiverse-access humility, individuation-invariant application, capability-orientation-independence understanding, awe-acknowledgment-vs-authorization-claim distinction. A custom eval suite that probes these directly is the cleanest test of "does ST work *as ST*?" not just "does ST happen to reduce general deception?"
...
### US-3: Synthetic document generation pipeline (SDFT data prep)
...
Deliverable: SDFT-ready training corpus (~1M tokens of validated synthetic documents) ready for Tier 0+ fine-tuning runs.

### US-4: Tier 0 — Prompted-context evaluation + cloud fine-tuning (immediate, ~$3-5K, 8-10 weeks)

**Phase A — Prompted-context evaluation:**
...
- [ ] Run all 4 context-injection conditions (Bare / SOUL / SOUL+PARENT / SOUL+PARENT+V6) on **high-deception-baseline target models**:
  - **Qwen 2.5 32B-Instruct** (high deception baseline per recent evals, less aggressive RLHF than Western frontier labs)
  - **Qwen 2.5 7B-Instruct** (cheap pipeline-validation, also runs locally on A3500 in 4-bit)
  - **Qwen 2.5 72B-Instruct** (mid-scale alternative, frontier open-source from non-Western tradition)
  ...
  - **Llama 3.1 405B** (Apollo-confirmed in-context scheming; only available frontier open-source)
...
Tier 0 is the cheapest, fastest, and lowest-risk way to ask both questions ST cares about: *does prompted-context move deception rates?* and *does fine-tuning move them further?* With Databricks Mosaic / Azure ML / Snowflake Cortex affordable at $150-500 per fine-tune cycle — and DXC partnership pricing likely better — we can do **both prompted-context evaluation AND the V6 §11.8 step-change fine-tuning test entirely on cloud at Tier 0**, without waiting for local hardware.
...
**Why Qwen 2.5 family as primary.** Recent evaluations (community-published deception benchmarks, Apollo-style probes applied to multiple model families) show Qwen 2.5 7B-Instruct and especially 32B-Instruct exhibiting *higher* deception/scheming rates than equivalent-sized Llama models. The likely cause is less aggressive Western-style RLHF in the post-training. This makes Qwen 2.5 the better experimental target than Llama for measuring ST-effect: more headroom for the framework to move the metric. Qwen 2.5 also has 7B (cheap), 32B (sweet-spot for fine-tuning cost-vs-capability), and 72B (mid-frontier scale) variants — a clean ladder for multi-scale testing.

**Why include Llama 3.1 405B at Tier 0 if Qwen is primary.** 405B is the only open-source model at frontier scale, and Apollo's late-2024 paper specifically documented in-context scheming in it. Including 405B in prompted-context evaluation gives us frontier-scale baseline data even before Tier 2 fine-tuning attempts. We can prompt-test it cheaply via Azure AI Foundry.
...
**Databricks vs. Azure ML vs. Snowflake Cortex for fine-tuning.** All three are viable; Databricks Mosaic is the strongest research-friendly choice (MosaicML's training stack is excellent, end-to-end workflow, reproducibility, supports Qwen). DXC's Databricks partnership likely gets preferential pricing — exploit it. Azure ML is fallback if Databricks is unavailable. Snowflake Cortex is more limited (Llama family primarily) — less ideal since we want Qwen as primary target.

**LoRA and QLoRA.** Full fine-tuning of a 32B model means updating all 32 billion parameters, which requires ~100-200 GB of GPU memory. **LoRA (Low-Rank Adaptation)** adds a small set of trainable "adapter" parameters to the base model and only updates those — typically 0.1-1% of original parameter count, base model frozen. **QLoRA** combines this with quantization: base model stored at 4-bit instead of 16-bit, reducing memory ~4×, while adapters train at full precision. The result: 32B fits comfortably on a single H100 80GB or 2× A100 80GB with QLoRA. Behavioral effect is comparable to full fine-tuning for most purposes. Cloud fine-tuning services (Databricks, Azure ML) handle this automatically.
...
Deliverable: working eval pipeline, prompted-context effect-size estimates per model per condition per benchmark, fine-tuning effect-size estimates at 32B (and possibly 72B) scale (the V6 §11.8 step-change test), methodology document. **This artifact set justifies Tier 1 (local hardware iteration acceleration) AND Tier 2 (frontier-scale H200) access.** Even a clean null result is publishable and decision-relevant — it tells us prompted-context and 32B/72B fine-tuning don't capture the V6 §11.8 effect, which is informative about where the real lever is.

### US-5: Tier 1 — Local hardware iteration acceleration (optional, conditional on access, ~$0 marginal)

**Tasks:**

- [ ] Verify Tier 1 hardware: A100 80GB vs H100 80GB vs H100 94GB (affects iteration capacity)
- [ ] Set up fine-tuning environment locally (HuggingFace TRL, LLaMA-Factory or Axolotl, vLLM for inference)
...
In the original plan I positioned Tier 1 (local hardware) as the *enabling tier* for fine-tuning, blocking cloud at prompted-context. With cloud fine-tuning affordable at $150-500 per cycle, **fine-tuning at 32B and 72B scale happens at Tier 0 in the cloud**. Tier 1 hardware is no longer a *prerequisite* for the V6 §11.8 step-change test.

What Tier 1 *does* enable: **iteration acceleration**. Cloud fine-tuning costs $150-500 per cycle. If we need 5-10 hyperparameter trials to find the best fine-tuning recipe, that's $750-5000 — meaningful. Local hardware costs only electricity, so trials are effectively free once setup is paid. Same with eval re-runs: each cloud-API eval pass costs real money; local eval is free.
...
**Hardware verification.** A100 80GB and H100 80GB both fit Qwen 2.5 32B QLoRA comfortably; H100 94GB even better; both support 72B QLoRA at 4-bit with tight headroom. A100 40GB or H100 40GB would force smaller models for local iteration. Verify before committing dev time to Tier 1 setup.

Deliverable: faster + cheaper iteration on methodology refinement; pre-validated Tier 2 plan executable in H200 burst.

### US-6: Tier 2 — Frontier-scale execution on 8× H200 (earned, 2-3 days GPU time, weeks of preparation)

**Tasks:**

- [ ] Refine all Tier 0/1 methodology to production quality (every script tested, every hyperparameter settled, every artifact-capture pipeline working)
...
- [ ] **Headline experiment:** QLoRA fine-tune Llama 3.1 405B on ST corpus + context-cultivation pattern (V6 §11.8 hypothesis at near-frontier scale; only available frontier open-source)
- [ ] **Companion experiment 1:** Full BF16 fine-tune of Qwen 2.5 72B-Instruct (vs. QLoRA from Tier 0 — answers whether QLoRA was undermining results, plus tests Qwen at largest mid-scale)
- [ ] **Companion experiment 2:** Full BF16 fine-tune of Qwen 2.5 32B-Instruct (replicates Tier 0 result with full fine-tune; checks QLoRA-vs-full-FT effect at primary target scale)
...
**Why full fine-tune the 32B and 72B Qwen models.** QLoRA is parameter-efficient but it's a constrained update — only the LoRA adapter parameters move; the base model stays frozen. A "full fine-tune" updates all base-model parameters, which is the gold-standard but requires significantly more memory and time. Doing the 32B/72B at full fine-tune on H200 (alongside the 405B QLoRA) tells us whether QLoRA was *constraining* the Tier 0 result — i.e., whether the effect we observed at Tier 0 underestimates the true effect.

**Why Llama 3.1 405B for headline despite Qwen-primary at Tier 0.** Qwen 2.5 tops out at 72B. For frontier-scale testing of V6 §11.8's ASI-scale claim, we need the largest available open-source model, which is Llama 3.1 405B. So the model strategy is: Qwen 2.5 32B/72B as primary fine-tuning targets at Tier 0 (better deception baseline, smaller and cheaper), Llama 3.1 405B as Tier 2 frontier-scale demonstration target (only available scale).
...
### US-7: Stretch — Institutional grant application & scaled validation ($100K-class)
...
1. **Capability progression matches resource progression — but cloud fine-tuning collapses Tier 0/1 partially.** Tier 0 (cloud only) now tests *both* prompted-context AND fine-tuning at 32B/72B scale, because Databricks Mosaic / Azure ML / Snowflake Cortex make fine-tuning affordable at $150-500 per cycle. Tier 1 (local hardware) becomes *iteration acceleration* rather than the fine-tuning gate. Tier 2 (frontier compute) tests the strongest claim (does V6 §11.8 step-change appear at near-ASI scale on Llama 3.1 405B?). Each tier's claim is informed by the prior tier's methodology being validated.

2. **Each tier produces gatekeeper-ready artifacts.** Tier 0 produces multi-model effect-size data + 32B/72B fine-tuning step-change result + methodology document, presentable to grant gatekeepers AND H200-access gatekeepers. Tier 1 produces faster-iteration ablations and refined methodology, presentable as evidence-of-thoroughness for Tier 2. Tier 2 produces frontier-scale demonstration, publishable as research.

### Why this isn't just "scale the experiment up gradually"

A naive plan would be: "run experiment small, run experiment medium, run experiment large." That works if the only variable is scale. Here, the *intervention itself* is multi-layered (corpus + context-cultivation, prompted vs fine-tuned, V6 §11.8's step-change is specifically about the *interaction*), so each tier tests a structurally different version of the hypothesis. Tier 0 tests both "does prompted-context work?" and "does fine-tuning at 32B/72B amplify the effect?" Tier 1 (local) accelerates iteration on what Tier 0 already tested. Tier 2 tests "does the step-change appear at near-ASI scale (405B), and does full-fine-tune-vs-QLoRA affect it?"
...
**Tier 0 budget target: ~$3-5K, capped at $7K.** Higher than originally planned because cloud fine-tuning is now in scope at this tier. The cap matters because going significantly above creates institutional friction (DXC budget approvals, etc.) that slows momentum. Discipline is engineered through: DXC partnership pricing exploitation upfront, small benchmark sizes initially before scale-up, careful model selection (Qwen 2.5 32B as primary keeps fine-tuning cycles affordable; avoid burning budget on low-headroom RLHF'd models), local A3500 development environment to minimize per-API-call dev costs, batch evaluation where possible, intermediate-result capture so failed runs don't re-run.

Tier 1 marginal cost: ~$0 for compute (local hardware), saves cloud spend on iteration cycles that would otherwise cost $150-500 each.
...
F-007 captures V6 doctrinal review (the corpus content that this research tests). F-008 captures deferred corpus-cleanup items (non-blocking for this research). F-009 (this feature) captures the empirical research execution. F-007 review should complete before US-4 Phase B (cloud fine-tuning) commits, because doctrinal corrections during F-007 review may affect what we fine-tune on. F-007 review can run in parallel with US-1/US-2/US-3/US-4 Phase A (these don't depend on doctrinal stability, only on having *some* corpus to inject for prompted-context).
